...
__device__ curandState* d_state; //
...
int main(int argc, const char* argv[])
{
    ...
    Property<curandState> state{n_cells, "state"};
    cudaMemcpyToSymbol(d_state, &state.d_prop, sizeof(d_state));

    // curandState* d_state;
    // cudaMalloc(&d_state, n_max * sizeof(curandState));
    auto seed = time(NULL);
    setup_rand_states<<<(n_cells + 128 - 1) / 128, 128>>>(n_cells, seed, state.d_prop);


    // Integrate positions
    Vtk_output output{"turing_w_noise"};
