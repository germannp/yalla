...
public:
        ...
        : n_max{n_max}, Computer<Pt>{n_max, args...}
    {
        ...
        cudaMalloc(&d_X2, n_max * sizeof(Pt));   // Heun step in rk12
        cudaMalloc(&d_absmax, n_max * sizeof(float));
        ...
        while (t < dt) {
            ...
            do {
                ...
                Computer<Pt>::template pwints<pw_int, pw_friction>(
                    n, d_X1, d_dX1, d_old_v, d_sum_v, d_sum_friction);
                add_rhs<<<(n + 32 - 1) / 32, 32>>>(
                    n, d_sum_v, d_sum_friction, d_dX1);
                Pt fix_dX1;
                if (fix_com) {
                    fix_dX1 = thrust::reduce(
                                  thrust::device, d_dX1, d_dX1 + n, Pt{0}) /
                              n;
                } else {
                    cudaMemcpy(&fix_dX1, &d_dX1[fix_point], sizeof(Pt),
                        cudaMemcpyDeviceToHost);
