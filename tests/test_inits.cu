...
std::array<Pt, n_max> store(Solution<Pt, n_max, Solver>& points)
{
...
template<typename Pt, int n_max, template<typename, int> class Solver>
float mean_dist_to_nbs(Solution<Pt, n_max, Solver>& points, float cut_off = 1)
{
...
const char* test_relaxation()
{
    ...
    Solution<float3, n_max, Grid_solver> points;

    relaxed_sphere(r_mean, points);
    auto pos_before = store(points);
    points.take_step<relu_force>(dt);
    points.copy_to_host();
    auto pos_after = store(points);
    ...
    pos_before = store(points);
    points.take_step<relu_force>(dt);
    points.copy_to_host();
    pos_after = store(points);
...
const char* test_cuboid_dimensions()
{
    ...
    auto mins = thrust::reduce(
        points.h_X, points.h_X + *points.h_n, points.h_X[0], compwise_min<float3>);
    ...
    auto maxs = thrust::reduce(
        points.h_X, points.h_X + *points.h_n, points.h_X[0], compwise_max<float3>);
