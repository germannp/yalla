...
// Wrapper to check for CUDA errors, see https://devblogs.nvidia.com/parallelforall/
// how-query-device-properties-and-handle-errors-cuda-cc/
inline void cudaErrorCheck(const char *file, int line) {
    ...
    if (syncError != cudaSuccess) {
        printf("Sync CUDA error: %s, %s(%d).\n", cudaGetErrorString(syncError), file, line);
        exit(-1);
    }
