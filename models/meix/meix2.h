...
Meix::Meix(std::string file_name) {
    ...
    while (count < n_facets) {
        ...
        Triangle T(Vertices[stoi(items[1])],Vertices[stoi(items[2])],Vertices[stoi(items[3])]);
        Facets.push_back(T);
        items.clear();
        count++;
    }

    //we want to construct the list of triangles adjacent to each vertex (vector of vectors)
