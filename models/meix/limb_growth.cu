...
void link_forces_w_n_init(Links<n_links>& links, const Pt* __restrict__ d_X, Pt* d_dX) {
    thrust::fill(thrust::device, n_epi_nbs.d_prop, n_epi_nbs.d_prop + n_max, 0);
    thrust::fill(thrust::device, n_mes_nbs.d_prop, n_mes_nbs.d_prop + n_max, 0);
...
int main(int argc, char const *argv[]) {
    ...
    for (auto time_step = 0; time_step <= n_time_steps; time_step++) {
        ...
        update_protrusions<<<(protrusions.get_d_n() + 32 - 1)/32, 32>>>(limb.get_d_n(),
            grid.d_grid, limb.d_X, protrusions.d_state, protrusions.d_link);
